...
#include "TagAndProbeAnalysis/FitModels/RooCMSShape.h"
...
CErfExpo::CErfExpo(RooRealVar &m, const Bool_t pass)
{
  ...
  if(pass) {
    sprintf(vname,"alfa%s",name);  alfa  = new RooRealVar(vname,vname,5,1,100);
    sprintf(vname,"beta%s",name);  beta  = new RooRealVar(vname,vname,0.01,0,1.5);
    sprintf(vname,"gamma%s",name); gamma = new RooRealVar(vname,vname,0.2,0,1);
  } else {
    sprintf(vname,"alfa%s",name);  alfa  = new RooRealVar(vname,vname,65.0,1,200);
    sprintf(vname,"beta%s",name);  beta  = new RooRealVar(vname,vname,0.04,0,0.5);
    sprintf(vname,"gamma%s",name); gamma = new RooRealVar(vname,vname,0.04,0,1);
  }  
